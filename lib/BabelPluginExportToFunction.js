...
const buildFunctionWrapperVar = babelTemplate(`function ${WRAPPER_ID}(FUNCTION_IDENTIFIERS) {
    'use strict';

    var ${CONFIG_ID} = CONFIG
    BODY
}`);
const buildModuleWrapperVar = babelTemplate(`module.exports = function ${WRAPPER_ID}(FUNCTION_IDENTIFIERS) {
    'use strict';

    var ${CONFIG_ID} = CONFIG
    BODY
};`);
const buildFunctionWrapperConst = babelTemplate(`function ${WRAPPER_ID}(FUNCTION_IDENTIFIERS) {
    'use strict';

    const ${CONFIG_ID} = CONFIG
    BODY
}`);
const buildModuleWrapperConst = babelTemplate(`module.exports = function ${WRAPPER_ID}(FUNCTION_IDENTIFIERS) {
    'use strict';
            ...
            Program: {
                ...
                exit(path) {
                    if (!this.isWrapped) {
                        ...
                        let wrapMethod = buildFunctionWrapperVar;

                        if (options.nodeVersion >= 8) {
                            wrapMethod = options.commonjs ? buildModuleWrapperConst : buildFunctionWrapperConst;
                        } else if (options.commonjs) {
                            wrapMethod = buildModuleWrapperVar;
                        }
